      ...
      <div style={{ maxWidth: '85%', borderRadius: 16, padding: 12, ...bubbleStyle(role) }}>
        ...
        {role !== 'user' && (message?.contextSummary || message?.activeEntityLabel || message?.screenLabel || message?.replyMode || message?.followUpPrompt) ? (
          <div style={{ marginTop: 8, fontSize: 12, opacity: 0.9, display: 'grid', gap: 4 }}>
            {!isSimpleMode && message?.screenLabel ? <div>Ekran: {message.screenLabel}</div> : null}
            {!isSimpleMode && message?.activeEntityLabel ? <div>Seçili kayıt: {message.activeEntityLabel}</div> : null}
            {message?.contextSummary ? <div>{message.contextSummary}</div> : null}
            {!isSimpleMode && message?.replyMode ? <div>Yanıt biçimi: {message.replyMode}</div> : null}
            {!isSimpleMode && message?.roleMode ? <div>Mod: {message.roleMode === 'SIMPLE' ? 'Sade' : 'Operasyon'}</div> : null}
