...
export function RoomPendingSection({ pendingStatus, setPendingStatus, pendingQ, setPendingQ, onlyAgreement, setOnlyAgreement, pendingFiltered, ...props }) {
  return <div className="card"><h3>Bekleyen Talepler</h3><div className="toolbarLeft" style={{ marginBottom: 10 }}><select value={pendingStatus} onChange={(e) => setPendingStatus(e.target.value)}><option value="OPEN">Açık (DRAFT + REQUESTED)</option><option value="REQUESTED">REQUESTED</option><option value="DRAFT">DRAFT</option></select><input value={pendingQ} onChange={(e) => setPendingQ(e.target.value)} placeholder="Ara (id / company / plate / driver / not)" style={{ minWidth: 280 }} /><label className="muted" style={{ display: "flex", gap: 6, alignItems: "center" }}><input type="checkbox" checked={onlyAgreement} onChange={(e) => setOnlyAgreement(e.target.checked)} />Sadece Agreement shiftleri</label><button type="button" className="btn sm" onClick={() => { setPendingQ(""); setOnlyAgreement(false); }}>Temizle</button></div>{pendingFiltered.length ? <table className="tbl"><thead><tr><th>ID</th><th>Company</th><th>Start</th><th>End</th><th>Harita</th><th>Teklif / Pazarlık</th><th>Vehicle + Driver</th><th>Approve</th><th>Reddet</th></tr></thead><tbody>{pendingFiltered.map((shift) => <RoomPendingShiftRow key={shift.id} shift={shift} {...props} />)}</tbody></table> : <div className="muted">Bekleyen talep yok.</div>}</div>;
}
...
export function RoomFinalListSection({ listStatus, setListStatus, listQ, setListQ, onlyAgreement, setOnlyAgreement, copilotShift, listFiltered, items, ...props }) {
  return <div className="card"><h3>Tüm Shifts</h3><div className="muted" style={{ marginBottom: 8 }}>Bekleyen Talepler üstte görünür. Bu listede varsayılan olarak sadece onaylı ve aktif işler gösterilir.</div><div className="toolbarLeft" style={{ marginBottom: 10 }}><select value={listStatus} onChange={(e) => setListStatus(e.target.value)}><option value="OPEN">Açık (APPROVED + ACTIVE)</option><option value="ALL">Hepsi</option><option value="REQUESTED">REQUESTED</option><option value="DRAFT">DRAFT</option><option value="APPROVED">APPROVED</option><option value="ACTIVE">ACTIVE</option><option value="DONE">DONE</option><option value="REJECTED">REJECTED</option></select><input value={listQ} onChange={(e) => setListQ(e.target.value)} placeholder="Ara (id / company / plate / driver / not)" style={{ minWidth: 320 }} /><label className="muted" style={{ display: "flex", gap: 6, alignItems: "center" }}><input type="checkbox" checked={onlyAgreement} onChange={(e) => setOnlyAgreement(e.target.checked)} />Sadece Agreement shiftleri</label><button type="button" className="btn sm" onClick={() => { setListQ(""); setListStatus("OPEN"); setOnlyAgreement(false); }}>Temizle</button></div><ListSelectionBanner selectedLabel={copilotShift ? `Vardiya #${copilotShift.id}` : ""} selectedSummary={copilotShift ? [String(copilotShift?.status || "").toUpperCase() || "-", fmtTR(copilotShift?.startAt), fmtTR(copilotShift?.endAt)].filter(Boolean).join(" • ") : ""} visibleCount={listFiltered.length} totalCount={items.length} filterValue={`${listQ} ${listStatus} ${onlyAgreement ? "agreement" : ""}`.trim()} onClearFilter={() => { setListQ(""); setListStatus("OPEN"); setOnlyAgreement(false); }} helper="Copilot seçili vardiyayı kullanır." />{listFiltered.length ? <table className="tbl"><thead><tr><th>ID</th><th>Status</th><th>Company</th><th>Teklifler</th><th>Vehicle</th><th>Driver</th><th>Start</th><th>End</th><th>Uzatma</th><th>Operasyon</th></tr></thead><tbody>{listFiltered.map((shift) => <RoomAllShiftRow key={shift.id} shift={shift} {...props} />)}</tbody></table> : <div className="muted">Kayıt yok.</div>}</div>;
}
