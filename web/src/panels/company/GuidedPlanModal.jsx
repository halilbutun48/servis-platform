...
export default function GuidedPlanModal({
  ...
  async function createDraftShifts() {
    ...
    try {
      ...
      for (const it of items) {
        const startAt = ymdMinToIso(startDate, it.startMin);
        const endYmd = it.endMin < it.startMin ? addDaysISO(startDate, 1) : startDate;
  ...
  async function osrmReorder(shiftId) {
    ...
    const s = (draftShifts || []).find((x) => Number(x.id) === sid);
    if (!s) {
      setErr("Shift bulunamadı.");
      return;
    }
    const stops = Array.isArray(s?.stops) ? s.stops : [];
    if (stops.length < 2) {
      setErr("Sıralama için en az 2 durak gerekir.");
      return;
    }
    ...
    try {
      ...
      if (!t?.ok) {
        setErr("OSRM matrisi alınamadı (opsiyonel). Solver/OSRM kapalı olabilir.");
        return;
      }
      const durationsSec = t?.durationsSec;
      const distancesM = t?.distancesM;
      ...
      if (!solved?.ok || !Array.isArray(solved?.orderPointIds)) {
        setErr("Çözüm alınamadı (solver kapalı olabilir).");
        return;
      }

      // orderPointIds includes "depot" + stop ids
      ...
      {step === 1 ? (
        <div style={{ marginTop: 12, display: "grid", gap: 12 }}>
          ...
          <div className="row" style={{ justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
            ...
            <button type="button" onClick={createDraftShifts} disabled={busy}>Taslak shift oluştur</button>
          </div>
        </div>
      ) : null}
