...
MustContainText $bubble '!isSimpleMode && message?.replyMode' 'chat bubble hides verbose metadata in simple mode'
MustContainAny $bubble @('İpucu:','message?.followUpPrompt ? <div>{isSimpleMode ? `İpucu: ${message.followUpPrompt}` : message.followUpPrompt}</div> : null','message?.followUpPrompt') 'chat bubble uses simpler follow-up label'
MustContainText $panel '/room/hub' 'panel still includes room hub after D4'
MustContainText $panel '/company/georeview' 'panel still includes company georeview after D4'
MustContainText $panel '/driver/checkin' 'panel includes driver checkin after D4'
