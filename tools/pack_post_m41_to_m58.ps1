...
Write-Host ''
Write-StatusLine '=== POST-M41 EXTERNAL PACK RUNNER (M42 -> M58) ==='
...
if ($SkipM58) {
  Write-StatusLine 'INFO M58 final pilot readiness step skipped by caller.'
} else {
  Write-StatusLine 'INFO M58 repo-pilot readiness packi varsayilan olarak dahildir.'
}
Write-Host ''
...
if (-not $SkipM58) {
  $steps += @{
    Name = 'M58 FINAL PILOT READINESS'
    Script = 'tools/pack_m58_final_pilot_readiness.ps1'
    Args = @('-RepoRoot', $RepoRoot)
  }
}
...
Write-Host ''
if ($SkipM57Scaffold -and $SkipM58) {
  ...
  Write-StatusLine '=== POST-M41 EXTERNAL PACK RUNNER (M42 -> M58) PASS OK ==='
}
Write-Host ''
