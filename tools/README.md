...
## Güncel SSOT yönü
- Resmi pack-green çizgi `M50` seviyesine kadar doğrulanmıştır.
- M51 docs/backlog reset işlendi.
- M52 Import + Geo Pipeline ana akış olarak çalışır duruma geldi.
- M53 Stop & Route Productization başladı.
- M53.3 aktif odak: **Planlama Merkezi'ni tek oluşturma kaynağına indirmek** ve Stage-3 akışını sadeleştirmek.
- Yakın resmi rota: `M54 ROOM Dispatch Planner` → `M55 Reports + No-show` → `M56 KVKK Matrix + Mobile Hardening` → `M57 Final Pilot Readiness`.

## Hijyen / kural notları
- README yalnızca gerçekten var olan script adlarını taşır; planlanan script isimleri yazılmaz.
- Tek seferlik `apply_* / overlay_*` script'leri tools kökünde tutulmaz; legacy arşiv altına taşınır.
- Company tarafında oluşturma için tek kaynak Planlama Merkezi hedeflenir; Vardiyalar takip / operasyon tarafında kalır.
