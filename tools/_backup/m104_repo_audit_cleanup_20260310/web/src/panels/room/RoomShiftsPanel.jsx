...
function calcPaxFromPreviewPayload(data) {
  const stops = Array.isArray(data?.stops) ? data.stops : [];
  const people = Array.isArray(data?.people) ? data.people : [];
  const sumFromStops = stops.reduce((sum, s) => {
    const n = Number(
      s?.assignmentCount ??
      s?.count ??
      s?.passengerCount ??
  ...
  const summaryPax = Number(
    data?.summary?.totalPax ??
    data?.summary?.pax ??
    data?.summary?.passengerCount ??
    0
  );
  const peopleLen = Array.isArray(people) ? people.length : 0;
  return Math.max(sumFromStops, Number.isFinite(summaryPax) ? summaryPax : 0, peopleLen, 0);
}

function calcCapacityPlan(requiredPax, vehicleList) {
  const pax = Number(requiredPax || 0);
  if (!Number.isFinite(pax) || pax <= 0) {
    return {
      requiredPax: 0,
      totalCapacity: 0,
      maxSingleCapacity: 0,
      singleVehiclePossible: true,
      fleetEnough: true,
      minVehicleCount: 1,
    };
  }

  const caps = (vehicleList || [])
    .map((v) => Number(v?.capacity || 0))
    .filter((n) => Number.isFinite(n) && n > 0)
    .sort((a, b) => b - a);

  const totalCapacity = caps.reduce((sum, n) => sum + n, 0);
  const maxSingleCapacity = caps[0] || 0;

  let acc = 0;
  let minVehicleCount = null;
  ...
  return {
    ...
    totalCapacity,
    maxSingleCapacity,
    singleVehiclePossible: maxSingleCapacity >= pax,
    fleetEnough: totalCapacity >= pax,
    minVehicleCount,
  };
}
...
export default function RoomShiftsPanel() {
  ...
  // route-preview tabanlı kişi/kapasite meta (özellikle organization akışı için)
  const [paxMeta, setPaxMeta] = useState({}); // { [shiftId]: { requiredPax, loading, error } }
...
async function ensurePaxMeta(shift, force = false) {
  const sid = Number(shift?.id);
  if (!Number.isFinite(sid) || sid <= 0) return 0;

  const prev = paxMeta[sid];
  if (!force && prev && Number.isFinite(Number(prev.requiredPax))) {
    return Number(prev.requiredPax || 0);
  }

  setPaxMeta((p) => ({
    ...p,
    [sid]: { ...(p[sid] || {}), loading: true, error: "" },
  }));

  try {
    const data = await api(`/api/shifts/${sid}/route-preview`, { token });
    const requiredPax = calcPaxFromPreviewPayload(data);
    setPaxMeta((p) => ({
      ...p,
      [sid]: { requiredPax, loading: false, error: "" },
    }));
    return requiredPax;
  } catch (e) {
    const msg = String(e?.message || e || "");
    setPaxMeta((p) => ({
      ...p,
      [sid]: { ...(p[sid] || {}), loading: false, error: msg },
    }));
    return Number(prev?.requiredPax || 0);
  }
}
  ...
  async function approveShift(shift) {
    ...
    try {
      await api(`/api/shifts/${sid}/approve`, {
        method: "PUT",
        token,
        body: { vehicleId, driverId, requiredPax: Number(paxMeta[sid]?.requiredPax || 0) || undefined },
      });
          ...
          <table className="tbl">
            ...
            <tbody>
              {pendingFiltered.map((s) => {
                ...
                const requiredPax = Number(
                  paxMeta[sid]?.requiredPax ??
                  s?.requiredPax ??
                  s?.totalPax ??
                  s?.passengerCount ??
                  s?.summary?.totalPax ??
                  s?.routePreview?.totalPax ??
                  s?.preview?.totalPax ??
                  0
                );

                const capacityPlan = calcCapacityPlan(requiredPax, roomVehicles);
                const capacityShort = Math.max(0, requiredPax - selectedCapacity);
                const capacityInsufficient =
                  requiredPax > 0 && selectedCapacity > 0 && selectedCapacity < requiredPax;

                const paxInfo = {
                  loading: Boolean(paxMeta[sid]?.loading),
                  requiredPax,
                  selectedCapacity,
                  capacityShort,
                  ...
                  <tr key={s.id}>
                    ...
                    <td>
                      <div style={{ display: "grid", gap: 6 }}>
                        ...
                        {!isAgreement ? (<div>{renderCompanyOfferSummary(s)}</div>) : null}
...
{renderAvailLine(s, vId, effDriverId, autoDriverName)}
        ...
        <div className="muted" style={{ fontSize: 12 }}>
          {paxInfo.loading ? (
            <span>Kişi/kapasite hesaplanıyor…</span>
          ) : requiredPax > 0 ? (
            <>
              <b>Talep kişi:</b> {requiredPax}
              {selectedCapacity > 0 ? <> • <b>Seçili araç:</b> {selectedCapacity}</> : null}
              {capacityPlan.singleVehiclePossible ? (
                <> • Tek araçla karşılanabilir.</>
              ) : capacityPlan.fleetEnough ? (
                <> • Tek araç yetmez; mevcut boş filoda en az <b>{capacityPlan.minVehicleCount}</b> araç gerekir.</>
              ) : (
                <> • Toplam boş kapasite yetersiz ({capacityPlan.totalCapacity}).</>
              )}
            </>
          ) : (
            <span>Kişi bilgisi yok.</span>
          )}
        </div>

        {capacityInsufficient ? (
          <div className="card err" style={{ padding: 10 }}>
            <b>Yetersiz kapasite</b>
            <div className="muted" style={{ marginTop: 6 }}>
              Gerekli: <b>{requiredPax}</b> • Seçili araç: <b>{selectedCapacity}</b> • Eksik: <b>{capacityShort}</b>
            </div>
          </div>
        ) : null}
</div>
                          ...
                          <div className="grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
                            ...
                            <div className="col">
                              ...
                              <div className="muted" style={{ display: "grid", gap: 6, marginTop: 10 }}>
                                ...
                                {Number(s?.agreementId) > 0 ? (
                                  <div>
                                    <b>Kaynak:</b> Agreement #{s.agreementId}
                                  </div>
                                ) : null}
                                <div>
