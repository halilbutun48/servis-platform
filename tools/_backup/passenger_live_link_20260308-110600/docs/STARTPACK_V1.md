...
## Unified Live Route (2026-03-08)
- ROOM / COMPANY / SCHOOL / DRIVER: tüm duraklar rota sırası ile gösterilir; harita + canlı araç + sıradaki durak highlight + sonraki durağa navigasyon + tam rotayı dış navigasyonda aç.
- PERSONEL: sadece kendi/önerilen durak odağında araç yaklaşımı + ETA + durağa navigasyon.
- DRIVER route active payload: `orderedStops` artık rota sırası (order asc); `proximityStops` mesafe sıralı fallback olarak ayrıca taşınır.
