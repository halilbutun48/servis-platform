...
export default function OrganizationPlansPanel() {
  ...
  useEffect(() => {
    if (!items.length || current.id) return;
    let selectedId = null;
    try { selectedId = Number(sessionStorage.getItem(PICK_KEY) || 0) || null; } catch {}
    const picked = selectedId ? items.find((x) => Number(x.id) === Number(selectedId)) : null;
    if (picked) {
      pick(picked);
  ...
  async function publishShift() {
    ...
    try {
      ...
      setMsg(`Markete açıldı. Shift #${data.shiftId}`);
      await load();
    } catch (e) {
      setMsg(String(e?.message || e));
    } finally {
