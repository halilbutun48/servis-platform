...
async function assertOrganization(req) {
  ...
  const company = await prisma.company.findUnique({
    where: { id: companyId },
    select: { id: true, kind: true, hubLat: true, hubLng: true, name: true },
  });
...
export function organizationRouter(io) {
  ...
  r.post("/plans/:id/publish-shift", async (req, res) => {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      await tx.organizationPlan.update({
        where: { id: plan.id },
        data: { status: "SHIFT_PUBLISHED", publishedShiftId: shift.id },
      });

      return shift;
    });

    io?.to?.(`company:${company.id}`)?.emit?.("organization:plan:update", { id: plan.id, kind: "shiftPublished", shiftId: created.id });
    io?.to?.(`company:${company.id}`)?.emit?.("shift:update", { id: created.id, kind: "created" });
    res.json({ ok: true, shiftId: created.id, roomId: null, market: true, status: "REQUESTED" });
  });
