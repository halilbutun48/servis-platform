...
async function main() {
  ...
  const orgPlan = await prisma.organizationPlan.upsert({
    ...
    update: {
      ...
      hubLat: 41.017,
      hubLng: 28.98,
      note: "Demo organization route plan",
    },
    create: {
      companyId: organization.id,
      name: "Fuar Giriş Dağıtım",
  ...
  await prisma.organizationStop.createMany({
    data: [
      { planId: orgPlan.id, name: "Fuar Giriş Kapısı", address: "İstanbul Fuar Merkezi", lat: 41.048, lng: 28.82, orderHint: 1, passengerCount: 12 },
      { planId: orgPlan.id, name: "Otel Transfer", address: "Yeşilköy Oteller Bölgesi", lat: 40.976, lng: 28.824, orderHint: 2, passengerCount: 8 },
      { planId: orgPlan.id, name: "Kongre Merkezi", address: "Büyükçekmece", lat: 41.02, lng: 28.58, orderHint: 3, passengerCount: 20 },
    ],
  });
