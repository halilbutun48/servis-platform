...
- Operasyon bölgesi `backend/src/region/operationRegion.js` üzerinden route/service-area kanıtı varsa önce, sonra aktif vardiya/sözleşme coğrafyası ve canonical COMPANY/ROOM bölgesi sırasıyla otomatik çözülür. Koordinattan sessiz reverse-geocode, IP/device konumu, sessiz İstanbul veya fabricated province kullanılmaz. Çözülen il #2 EPDK katmanına taşınır; yakıt katmanı mevcutken sürücü/bakım/platform/actual eksikliği dış referansı saklamaz.
- `Piyasa referansı kısmen hazır` durumunda dış yakıt değeri, provider/kaynak, kapsam, as-of ve güncellik; `Reference completeness`, yakıt, sürücü, bakım, platform gözlemi ve actual durumları ayrı gösterilir. Yakıt değeri varsa aynı #4 preview motoru partial monetary cost üretir; eksik sürücü/bakım maliyeti sonuçtan saklanmaz ve canlı mutasyon başlatılmaz.
