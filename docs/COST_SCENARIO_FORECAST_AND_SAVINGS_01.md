...
- Canonical yerleşim: COMPANY senaryo yüzeyi `Bütçe ve Servis Maliyeti → Maliyet Senaryosu`, ROOM senaryo yüzeyi `Teklif ve Kârlılık → Maliyet Senaryosu` içindedir. COMPANY veya ROOM sol menüsünde ayrı `Maliyet Senaryoları` maddesi yoktur. Teknik deep-link (`/company/cost-scenarios`, `/room/cost-scenarios`) test, reusable owner ve geriye dönük erişim için korunur. SCHOOL/ORGANIZATION `Planlama Senaryosu` ayrı planning-only yüzeydir; finans rotası açılmaz.
