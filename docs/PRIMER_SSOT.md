...
- Bu değişiklik yapılırsa **yeni bir tag** ile sabitlenmesi önerilir (mevcut `v1-m13-green.2` referansını kirletmemek için).
...
“`servis-platform` repo, tag `v1-m13-green.2` referansından devam ediyoruz. Next hedef: Vehicle↔Driver bind + overlap rules. API + UI görevlerini sıraya koy; Gate/Pack standardını bozmadan ilerle.”
