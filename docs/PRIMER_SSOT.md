...
- `#4 COST-SCENARIO-FORECAST-AND-SAVINGS-01` `docs/COST_SCENARIO_FORECAST_AND_SAVINGS_01.md`, `backend/src/finance/costScenarioForecast.js`, `backend/src/routes/costScenario.js`, `web/src/panels/shared/CostScenarioWorkspacePanel.jsx` ve `check:costscenarioforecastandsavings01` ile yaşar; `operationalCostModel` tek hesap sahibidir, sonuç ephemeral ve preview-only'dir.
...
- `DATABASE-BACKUP-RETENTION-AND-INTEGRITY-01` PostgreSQL custom-format backup, SHA-256 checksum, retention inventory ve yalnızca izole restore rehearsal için tek canonical owner'dır; `check:databasebackupretentionandintegrity01`, `docs/DATABASE_BACKUP_RETENTION_AND_INTEGRITY_01.md`, `backend/src/ops/databaseBackupService.js` ve `backend/scripts/database_backup_retention_and_integrity_01_acceptance.mjs` ile yaşar; M45 backup adları bu owner'a delegedir, canonical DB overwrite, reset/reseed, payment/accounting write ve protected runtime-data erişimi açmaz.
