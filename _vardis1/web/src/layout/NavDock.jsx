...
export default function NavDock({ role, path, me }) {
  ...
  const roleLabel = role === "COMPANY" && me?.companyKind === "SCHOOL" ? "SCHOOL" : role === "COMPANY" && me?.companyKind === "ORGANIZATION" ? "ORGANIZATION" : role;

  return (
    <div className="navDock">
      <div className="navDockTitle">
        <div>Vardis</div>
        <div className="muted" style={{ fontSize: 12, fontWeight: 500, marginTop: 4 }}>{roleLabel}</div>
      </div>
