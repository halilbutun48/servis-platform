...
async function main() {
  ...
  await prisma.user.updateMany({
    ...
    data: { totpSecretBase32: null, totpPendingSecretBase32: null, totpEnabledAt: null, totpLastVerifiedAt: null },
  });
  ...
  const roomPre = await reqJson("POST", "/api/drivers", { token: roomLogin.token, body: { name: `Step1 Room Driver ${Date.now()}`, phone: "05000000001" }, includeGreenpack: false });
  ...
  const roomSetup = await reqJson("POST", "/api/auth/totp/setup", { token: roomLogin.token, body: {}, includeGreenpack: false });
  must("room totp setup ok", roomSetup.ok && !!roomSetup.json?.secretBase32);
  const roomCode = totpToken(roomSetup.json.secretBase32);
  const roomEnable = await reqJson("POST", "/api/auth/totp/enable", { token: roomLogin.token, body: { code: roomCode }, includeGreenpack: false });
  must("room totp enable ok", roomEnable.ok && roomEnable.json?.enabled === true);

  const roomNoStepUp = await reqJson("POST", "/api/drivers", { token: roomLogin.token, body: { name: `Step1 Room Driver B ${Date.now()}`, phone: "05000000002" }, includeGreenpack: false });
  ...
  const roomVerify = await reqJson("POST", "/api/auth/totp/verify", { token: roomLogin.token, body: { code: totpToken(roomSetup.json.secretBase32) }, includeGreenpack: false });
  ...
  const roomAllowed = await reqJson("POST", "/api/drivers", { token: roomVerify.json.token, body: { name: `Step1 Room Driver C ${Date.now()}`, phone: "05000000003" }, includeGreenpack: false });
  must("room sensitive write allowed after verify", roomAllowed.ok && !!roomAllowed.json?.id);
  ...
  const superSetup = await reqJson("POST", "/api/auth/totp/setup", { token: superLogin.token, body: {}, includeGreenpack: false });
  must("super totp setup ok", superSetup.ok && !!superSetup.json?.secretBase32);
  const superEnable = await reqJson("POST", "/api/auth/totp/enable", { token: superLogin.token, body: { code: totpToken(superSetup.json.secretBase32) }, includeGreenpack: false });
  ...
  const superVerify = await reqJson("POST", "/api/auth/totp/verify", { token: superLogin.token, body: { code: totpToken(superSetup.json.secretBase32) }, includeGreenpack: false });
