...
console.log("=== PAY-01D PAYMENT PREVIEW CSV EXPORT CHECK ===");
...
must(rootPkg, '"check:pay01d": "node backend/scripts/pay_01d_payment_preview_csv_export_check.js"', "root package exposes check:pay01d");
...
must(previewCard, "CSV taslağı indir", "payment preview card keeps csv download action");
must(previewCard, "Sadece önizleme verisi indirilir.", "payment preview card keeps readonly csv note");
...
must(api, "getPaymentBackboneReadinessPreviewCsv", "api exposes payment backbone preview csv helper");
...
must(route, 'r.get("/payment-backbone/readiness/preview.csv", authRequired(), requireRole("SUPER_ADMIN"), async (req, res) => {', "commercial core route mounts readonly payment preview CSV GET endpoint");
...
must(route, "ilgili_kayit", "payment preview csv header includes related record column");
must(route, "sozlesme_veya_vardiya", "payment preview csv header includes contract or shift column");
must(route, "komisyon_durumu", "payment preview csv header includes commission status column");
must(route, "odeme_hesabi_durumu", "payment preview csv header includes payment account column");
must(route, "kontrol_notu", "payment preview csv header includes control note column");
must(route, "taslak_tutar", "payment preview csv header includes draft amount column");
must(route, "durum", "payment preview csv header keeps status column");
must(route, "join(\",\")", "payment preview csv header is assembled as csv");
mustNot(route, "token", "payment preview csv route does not expose token wording");
mustNot(route, "hash", "payment preview csv route does not expose hash wording");
...
mustNoMigrationMarker("pay01d", "no pay-01d migration folder detected");
...
console.log("=== PAY-01D PAYMENT PREVIEW CSV EXPORT CHECK PASS ===");
