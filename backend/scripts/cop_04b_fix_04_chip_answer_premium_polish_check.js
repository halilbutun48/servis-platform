...
assertProductExtensionsIncludes('check:cop04bfix04', 'product extensions registry references cop04bfix04', registryScripts);
assertProductExtensionsIncludes('check:cop04bfix04', 'verify chain registry waits for check:cop04bfix04', registryScripts);
...
must(answerPolicySource, 'konum sinyali güncel değil / çevrim dışı satırını aç', 'answer policy keeps stale chip');
...
must(answerPolicySource, 'Son konum bilgisi ne zaman geldi?', 'answer policy keeps gps chip');
...
mustAny(roomMapReply, ['konum sinyali: Zayıf / güncel değil', 'konum sinyali: Güncel değil', 'konum sinyali: Zayıf / STALE', 'konum sinyali: Çevrim dışı', 'konum sinyali: Bekleniyor'], 'room map reply keeps gps state');
mustAny(roomMapReply, ['Sürücünün telefonundan konum sinyali', 'Araç bağlantısı', 'görev bağlantısı'], 'room map reply keeps gps source language');
...
mustArrayContains(roomMapChips, 'Son konum bilgisi ne zaman geldi?', 'room map chips keep gps chip');
mustArrayContains(roomMapChips, 'Sürücünün telefonundan konum sinyali devrede mi?', 'room map chips keep driver gps chip');
...
mustAny(roomOperationReply, ['konum sinyali güncel değil / çevrim dışı 1', 'konum sinyali güncel değil / çevrim dışı: 1'], 'operation health reply keeps stale offline count');
mustAny(roomOperationReply, ['açık sorun 2', 'Açık sorun: 2'], 'operation health reply keeps open issue count');
must(roomOperationReply, 'Riskli cihazı aç, konum sinyali güncel değil / çevrim dışı satırını kontrol et ve açık sorunları sırala.', 'operation health reply keeps natural next step');
...
mustArrayContains(roomOperationChips, 'konum sinyali güncel değil / çevrim dışı satırını aç', 'operation health chips keep stale chip');
...
mustArrayContains(companyShiftChips, 'konum sinyali/operasyon kanıtını kontrol et', 'company shift chips keep proof chip');
...
mustArrayContains(personelChips, 'Son konum bilgisi ne zaman geldi?', 'personel chips keep gps chip');
mustArrayContains(personelChips, 'Servis durumu ne?', 'personel chips keep status chip');
mustArrayContains(personelChips, 'Sürücünün telefonundan konum sinyali devrede mi?', 'personel chips keep phone gps chip');
...
mustArrayContains(parentChips, 'Son konum bilgisi ne zaman geldi?', 'parent chips keep live service chip');
mustArrayContains(parentChips, 'Son konum bilgisi ne zaman geldi?', 'parent chips keep gps chip');
mustArrayContains(parentChips, 'Tahmini varış süresi nedir?', 'parent chips keep eta chip');
...
mustAny(driverReply, ['Sürücünün telefonundan konum sinyali', 'Telefon konum sinyali'], 'driver reply keeps gps source wording');
...
mustArrayContains(driverChips, 'durak nerede?', 'driver chips keep next stop chip');
mustArrayContains(driverChips, 'konum sinyali/operasyon kanıtını kontrol et', 'driver chips keep proof chip');
