...
async function main() {
  console.log("=== M26CHECK: Company workflow + agreement templates (API) ===");
  ...
  // directory: company can search rooms
  const rs = await mustOk(reqJson("GET", `/api/rooms?q=${encodeURIComponent(rand)}`, { token: tC }), "company rooms search");
  if (!Array.isArray(rs?.items)) throw new Error("rooms list -> items missing");
  ...
  if (!ag?.id) throw new Error("agreement create -> id missing");
  ok("agreement create");

  const al = await mustOk(reqJson("GET", "/api/agreements?take=50", { token: tC }), "agreements list");
  if (!Array.isArray(al?.items)) throw new Error("agreements list -> items missing");
  if (!al.items.some((x) => x.id === ag.id)) throw new Error("created agreement not found in list");
  ok("agreements list contains created");

  // cleanup: cancel
  await mustOk(reqJson("PUT", `/api/agreements/${ag.id}/cancel`, { token: tC, body: {} }), "agreement cancel");
  ok("agreement cancel (cleanup)");

  console.log("OK M26CHECK PASS");
}

main().catch((e) => {
  console.error("FAIL M26CHECK FAIL");
