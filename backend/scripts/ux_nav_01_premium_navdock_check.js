...
const appSource = read(APP_JSX_ROLE_TENANT_SCOPE_PATHS[0]);
  ...
  must(navDockSource, 'advanced.push({ label: "Taşımacılık Firması Konumu", path: "/room/hub" });', 'room konum item stays in advanced section');
  ...
  must(navDockSource, 'Organizasyon Operasyon Paneli', 'nav dock keeps organization operations item visible');
  ...
  must(navDockSource, 'Organizasyon Planları', 'nav dock keeps organization planning item visible');
  ...
  must(labelsSource, 'return "Hizmet Alan Firma Konumu";', 'labels helper keeps company konum label');
  ...
  must(screenRegistrySource, 'label: "Taşımacılık Firması Konumu"', 'screen registry uses room konum label');
  ...
  must(screenRegistrySource, 'label: "Organizasyon Planları"', 'screen registry uses organization planning label');
