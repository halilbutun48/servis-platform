...
ok(
  workflow.includes("if (!token || !guidedOpen) return;") && workflow.includes("getCompanyRooms(token"),
  "workflow room directory is gated by guided modal"
);
ok(
  !workflow.includes("loadRooms(controller.signal);\n      loadStats(controller.signal);"),
  "workflow initial effect no longer broad-loads rooms"
);
ok(
  workflow.includes("fetchProviderScoreMap(visibleOfferRoomIds, token)") ||
    workflow.includes("fetchProviderScoreMap(roomScoreIds, token)"),
...
ok(
  agreements.includes('const q = String(req.query.q || "").trim();') ||
    agreements.includes("const q = String(req.query.q || '').trim();"),
...
ok(
  offers.includes('const q = String(req.query.q || "").trim();') ||
    offers.includes("const q = String(req.query.q || '').trim();"),
...
ok(
  (
    hub.includes("getCompanyAgreements(token, { signal, force = false, q, take = COMPANY_DATA_TAKE.agreements, status, ttlMs = COMPANY_DATA_TTL.agreements") &&
    hub.includes("getCompanyOffers(token, { signal, force = false, q, take = COMPANY_DATA_TAKE.offers, status, ttlMs = COMPANY_DATA_TTL.offers")
  ) ||
  (
    hub.includes("q, take = COMPANY_DATA_TAKE.agreements, status, ttlMs") &&
    hub.includes("q, take = COMPANY_DATA_TAKE.offers, status, ttlMs")
  ),
...
ok(
  scale.includes("provider score backend batch endpoint exists and is used on web") ||
    scale.includes("provider score backend batch endpoint is used"),
...
ok(
  storm.includes("/api/trust-quality/provider-scores?roomIds=") || storm.includes("lazy provider scores") || storm.includes("M75 profile uses lighter first-load takes + offer/people/live-shift read buckets + lazy provider scores"),
