...
function main() {
  ...
  if (shifts.includes("shouldLoadRoomScores") && shifts.includes("offersModal?.items") && !shifts.includes("ProviderScoreBadge score={roomScores[String(r?.id || s.roomId || 0)] || null}")) ok("ShiftsPanel limits provider-score hot path to offer contexts"); else fail("ShiftsPanel limits provider-score hot path to offer contexts");
  ...
  if (mapPanel.includes("getShiftRoutePreview") && mapPanel.includes("ttlMs: 60000") && mapPanel.includes("take: 20")) ok("MapPanel uses shared route preview helper and lighter vehicle load"); else fail("MapPanel uses shared route preview helper and lighter vehicle load");
  if (routePreview.includes("getShiftRoutePreview") && routePreview.includes("ttlMs: 30000")) ok("RoutePreviewModal uses shared helper with longer cache"); else fail("RoutePreviewModal uses shared helper with longer cache");
  ...
  if (geo.includes("take: debouncedQ ? 30 : 16")) ok("GeoReviewPanel narrows fallback personel query"); else fail("GeoReviewPanel narrows fallback personel query");
  ...
  if (server.includes("readOfferLimiter") && server.includes("readPeopleLimiter") && server.includes("readLiveShiftLimiter")) ok("server defines dedicated offer/people/live-shift limiters"); else fail("server defines dedicated offer/people/live-shift limiters");
  if (server.includes("isOfferReadPath") && server.includes("isPeopleReadPath") && server.includes("isLiveShiftReadPath")) ok("server detects dedicated hot path classes"); else fail("server detects dedicated hot path classes");

  const storm = read("backend/scripts/company_fetch_storm_check.js");
  if (storm.includes("/api/shifts?take=32") && storm.includes("/api/rooms?take=30") && storm.includes("/api/vehicles?onlyActive=1&take=20") && storm.includes("M75 profile uses lighter first-load takes")) ok("storm profile updated to M75 values"); else fail("storm profile updated to M75 values");
