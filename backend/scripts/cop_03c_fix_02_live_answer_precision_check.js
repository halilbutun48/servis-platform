...
must(helpComposer, 'workflowAction = (() => {', 'help composer keeps workflow action gate');
...
must(helpComposer, "if (roleBoundary && ['ROLE_HELP', 'WHO_CAN_DO', 'ROLE_BOUNDARY', 'KVKK_VISIBILITY'].includes(String(questionType || ''))) chips.unshift('Yetki sınırını kontrol et');", 'help composer narrows role-boundary chip injection');
...
must(helpComposer, "if (path.includes('/room/map')) return ['Son GPS ne zaman geldi?', \"Sürücünün telefon GPS’i devrede mi?\", 'Araç bağlantısı var mı?', 'Canlı takip ekranını aç'];", 'help composer keeps GPS workflow chips without self-question');
...
mustNot(facts, 'raw', 'facts avoids raw wording');
