...
ok(includesText(shifts, 'if (trackTab === "contract") return contractItems[0] || null;'), 'company shifts picks first contract row for copilot selection');
ok(includesText(shifts, 'return otherItems[0] || null;'), 'company shifts picks first other row for copilot selection');
