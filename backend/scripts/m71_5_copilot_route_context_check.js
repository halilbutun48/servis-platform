...
if (has(copilot, 'setChatScreenId(String(match.id));') && has(copilot, '}, [hashPath, screenOptionsFromMe, sharedContext?.screenPath]);')) ok('CopilotPanel syncs chat screen on route change'); else fail('CopilotPanel syncs chat screen on route change');
if (has(composer, "const hasScreenContext = !preferEntityContext && (entityType === 'screen' || Boolean(screenContext?.path || screenDefinition?.path));")) ok('helpComposer widens screen context gate'); else fail('helpComposer widens screen context gate');
