...
async function main() {
  ...
  must("README points to master pack and docs pack", includesAny(readme, ["tools\\pack.ps1 -To 66", "tools\\pack_docs_ssot.ps1"]));
  must("primer reflects M66 functional honesty", includesAny(primer, ["M66", "fonksiyonel", "tam milestone kapanışı için"]));
  must("tools primer reflects M66 functional honesty", includesAny(toolsPrimer, ["M66", "fonksiyonel", "tam milestone kapanışı için"]));
  must("startpack points to master pack and docs pack", includesAny(startpack, ["tools\\pack.ps1 -To 66", "tools\\pack_docs_ssot.ps1"]));
  ...
  must("registry lists M59-M65 green and M66 functional/open", includesAny(registry, ["M66 - Operasyonel Reassignment - functional-open", "M66 — Operasyonel Reassignment — functional-open", "M66 - Operasyonel Reassignment - fonksiyonel / tekrar test acik", "M66 — Operasyonel Reassignment — fonksiyonel / tekrar test acik"]));
  must("backlog points to rerun + cleanup", includesAny(backlog, ["M0-M66", "cleanup", "saha testi"]));
  must("tools readme lists master and docs pack", includesAny(toolsReadme, ["tools\\pack.ps1 -To 66", "tools\\pack_docs_ssot.ps1"]));
  must("full runbook mentions master pack", includesAny(fullRunbook, ["tools\\pack.ps1 -To 66", "pack_docs_ssot"]));
