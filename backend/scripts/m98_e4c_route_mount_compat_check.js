...
function main() {
  console.log("=== M98-E4C ROUTE MOUNT COMPAT CHECK ===");
  ...
  assert(server.includes("mountCoreRoutes(app, {"), "server calls mountCoreRoutes");
  assert(server.includes("personelAccessRouter,"), "server passes personelAccessRouter into mountCoreRoutes");
  assert(server.includes("publicPersonelInviteRouter,"), "server passes publicPersonelInviteRouter into mountCoreRoutes");
  ...
  console.log("=== M98-E4C ROUTE MOUNT COMPAT CHECK PASS ===");
}
