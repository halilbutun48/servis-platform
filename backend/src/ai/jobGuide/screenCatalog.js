...
const SCHOOL = COMPANY.map((x) => ({ ...x, path: x.path.replace('/company', '/school') })).concat([
...
const ORGANIZATION = COMPANY.map((x) => ({ ...x, path: x.path.replace('/company', '/organization'), label: x.label === 'Planlama Merkezi' ? 'Organizasyon Merkezi' : x.label })).concat([
...
const DRIVER = [
  ...
  screen(3104, '/driver/copilot', 'Copilot', {
