...
export function commercialCoreRouter() {
  ...
  r.get("/payment-backbone/readiness/preview.csv", authRequired(), requireRole("SUPER_ADMIN"), async (req, res) => {
    const bucket = normalizePaymentPreviewBucket(req.query?.bucket);
    const queueRows = await listSettlementOperationQueue({ take: paymentPreviewTake });
    const sourceRows = queueRows.length
      ? queueRows
      : await listCommercialSources({ take: paymentPreviewTake });
    const sourceLabel = queueRows.length ? "hakediş kuyruğu" : "ticari omurga";
    const summary = buildPaymentPreviewSummary(sourceRows, sourceLabel);
    const visibleItems = bucket === "ALL"
      ? summary.items
      : summary.items.filter((item) => String(item?.status || "").toUpperCase() === bucket);
    const csvRows = buildPaymentPreviewCsvRows(visibleItems);
    await audit(req, {
      action: "PAYMENT_PREVIEW_EXPORT",
      entity: "PaymentPreview",
      meta: buildKvkkExportAuditMeta({
        endpoint: "/api/commercial-core/payment-backbone/readiness/preview.csv",
    ...
    const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-");
    res.setHeader("Content-Disposition", `attachment; filename="payment_preview_${stamp}.csv"`);
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    const header = ["durum", "ilgili_kayit", "sozlesme_veya_vardiya", "komisyon_durumu", "odeme_hesabi_durumu", "kontrol_notu", "taslak_tutar"].join(",");
    return res.send(`\ufeff${header}\n${csvRows.join("\n")}\n`);
  });
  ...
  r.get("/room/financial-operations/preview", authRequired(), requireRole("ROOM", "SUPER_ADMIN"), async (req, res) => {
    const roomId = Number(req.user?.roomId || req.query?.roomId || 0) || 0;
    if (roomId <= 0) {
      return res.status(400).json({
        ...
        message: "Room financial operations preview için roomId gerekli.",
      });
    }

    const previewUser = { ...req.user, roomId };
    const [room, roomSummary, latestShift, latestAgreement] = await Promise.all([
      ...
      prisma.shift.findFirst({
        where: { roomId, status: { in: ["APPROVED", "ACTIVE", "SPLIT"] } },
    ...
    if (!room) {
      return res.status(404).json({
        ...
        message: "Room financial operations preview için oda bulunamadı.",
      });
    }

    return res.json(buildRoomProfitabilityAndQuoteFloorPreview({
      role: req.user?.role,
