...
## 1) GOLDEN RULES
1. Ana resmi green referans `M56` seviyesine kadar dogrulanmis pack cizgisidir.
2. Post-M41 pack script'leri self-only calisir; tam `M42 -> M56` green hatti ve `M57` scaffold kontrolunun kanonik komutu `tools\pack_post_m41_to_m57.ps1 -RepoRoot D:\servis-platform -NoBuild` seklindedir.
3. Uyumluluk icin `tools\pack_post_m41_to_m54_4.ps1` dosyasi korunur; yeni orchestrator'a forward eder.
...
- M57.1 implementation pack: `tools\pack_m57_mobile_hardening.ps1 -RepoRoot D:\servis-platform`
- M57 scaffold pack: `tools\pack_m57_mobile_hardening.ps1 -RepoRoot D:\servis-platform -ScaffoldOnly`
- Post-M41 external orchestrator: `tools\pack_post_m41_to_m57.ps1 -RepoRoot D:\servis-platform -NoBuild`
- Compat wrapper: `tools\pack_post_m41_to_m54_4.ps1 -RepoRoot D:\servis-platform -NoBuild`
...
- Sonraki ana urun hatti `M57 — Mobile Hardening` olarak devam eder; M57.1 implementation check ve scaffold cizgisi repo icinde sabitlenmistir.
...
- Bu komut resmi green degildir; yalnizca M57 kapsam/runbook/check hazirligini dogrular.
